...
- удалять уведомления о начале и конце урока через 10 минут после отправки;
- редактировать существующие уроки через кнопку из раздела `Все записи`;
- сохранять отчет о уроке: имя фамилия, школа, оплата (автоподсчет в гривнах);
- хранить все в одном месте (общая SQLite база `bot_data.sqlite3` для двух пользователей).
...
- Кнопка `Удалить все записи` удаляет сразу все уроки и отчеты из общей базы.
...
- `Все записи` выводятся блоками: `дата -> школа -> ученики и время`.
